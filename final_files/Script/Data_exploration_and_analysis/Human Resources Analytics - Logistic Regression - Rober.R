...
#Now we are going to define an 80%/20% train/test split of the dataset.
...
#The predictive power is very good, with an 87% rate of success.

table(test$left, fitted.results, dnn = c('Actual Group','Predicted Group'))

##ROC curve

#We can use the ROC curve too, to analyze the predictive power.
...
#And now we can see the area under the ROC curve.
...
#We can see here again that is a good predictive model.

###Prediction with Cross Validation###

#Now we are going to see the rate of success of the model with a Cross Validation, to hacve a more reliable result.
...
#Test with only the significative variables
...
#Now all our variables are significant.
...
#And the area under the ROC curve too.
...
#But in the Cross Validation we can see that this model is sightly better than the former, so in this case, we could remove the "department" variable.